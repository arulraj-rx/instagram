...
class ThreadsPoster:
    def __init__(self, settings=None):
        ...
        self.processing_poll_attempts = int(
            settings.get("threads_processing_poll_attempts", 24)
        )
        self.publish_poll_attempts = int(
            settings.get("threads_publish_poll_attempts", 12)
        )
    ...
    def _post(self, url, payload, label):
        ...
        if response.status_code != 200:
            raise requests.HTTPError(f"{label}: {response.text}", response=response)
        return response
    ...
    def _publish_container(self, creation_id):
        ...
        response = self._post(url, payload, "Threads publish failed")
        thread_id = response.json().get("id")
        if not thread_id:
